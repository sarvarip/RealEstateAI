...
- **`Agent.iter()` over `Agent.run()`**: Discovered that `Agent.run()` doesn't allow early termination — after a tool returns, the agent keeps calling more tools. `Agent.iter()` gives step-by-step control, enabling immediate termination when the report pipeline completes.
